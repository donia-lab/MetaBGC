...
### Load full profiled cyclase TcmN HMMs for synthetic genomes
```{r, message = FALSE}
...
pks_genomes_median_check <-c("gi|640932211|gb|JJOH01000028.1|", "gi|640937269|gb|JJOH01000002.1|", "JOFH01000019.1", 
                             "gi|640930959|gb|JJOH01000095.1|", "AZXI01000002.1", "gi|529222834|ref|NC_021985.1|")
...
pks_genomes_subfive_check <-c("gi|1098134558|emb|FOXX01000014.1|", "gi|928414697|gb|LGKI01000478.1|", "AZXI01000002.1",
                              "gi|640930959|gb|JJOH01000095.1|", "gi|640932211|gb|JJOH01000028.1|", "gi|640937269|gb|JJOH01000002.1|",
                              "gi|529222834|ref|NC_021985.1|", "JOFH01000019.1", "AZXI01000013.1")
...
pks_genomes_plusfive_check <-c("JOFH01000019.1")
remaining_hmm_TcmN_pos_plusfive_check <- TcmN_filtered_median_plusfive %>% filter(readCheck == "hmm-unique-read") %>% separate(readID, c("scaffold_id", "rest_id"), sep = "-", remove = F) %>% filter(scaffold_id %in% pks_genomes_plusfive_check)
```
...
#parseReads(remaining_hmm_TcmN_pos_median_check, "cyclaseTcmN", "/Users/francinecamacho/Google Drive/T2PKS-paper/computational_analysis/synthetic_genomes/revision-synthetic-genomes/HMM-analysis/TcmN/full_model-analysis/hmm-unique-analysis/median_score/")

#parseReads(remaining_hmm_TcmN_pos_subfive_check, "cyclaseTcmN", "/Users/francinecamacho/Google Drive/T2PKS-paper/computational_analysis/synthetic_genomes/revision-synthetic-genomes/HMM-analysis/TcmN/full_model-analysis/hmm-unique-analysis/minus_five_score")

#parseReads(remaining_hmm_TcmN_pos_plusfive_check, "cyclaseTcmN", "/Users/francinecamacho/Google Drive/T2PKS-paper/computational_analysis/synthetic_genomes/revision-synthetic-genomes/HMM-analysis/TcmN/full_model-analysis/hmm-unique-analysis/plus_five_score")


```

```{r, message=FALSE}
#Load TcmN hmm-unique blast to TcmI domains reads to remove them from the F1 analysis 
TcmN_hmmunique_blast <- read_tsv("full_model-analysis/hmm-unique-analysis/TcmN_hmm-unique_blast-results.txt", col_names = T)
...
remove_TcmN_reads_subfive <- TcmN_hmmunique_blast %>% filter(cutoff == "minus_five_score") %>% select(qseqid, Sample) %>% distinct()
names(remove_TcmN_reads_subfive)[1] <- "readID"

remove_TcmN_reads_median <- TcmN_hmmunique_blast %>% filter(cutoff == "median_score") %>% select(qseqid, Sample) %>% distinct()
names(remove_TcmN_reads_median)[1] <- "readID"
remove_TcmN_reads_plusfive <- TcmN_hmmunique_blast %>% filter(cutoff == "plus_five_score") %>% select(qseqid, Sample) %>% distinct()
names(remove_TcmN_reads_plusfive)[1] <- "readID"

TcmN_f1_cutoff_median <-  calculate_F1(compareCyclaseReads(cyclaseTcmN_recoded %>% anti_join(., remove_TcmN_reads_median) %>% filter(HMMScore >=25),t2pksBlastDF)) #0.2139241
TcmN_f1_cutoff_plusfive <- calculate_F1(compareCyclaseReads(cyclaseTcmN_recoded %>% anti_join(., remove_TcmN_reads_plusfive) %>% filter(HMMScore >=30),t2pksBlastDF)) #0.2139241
TcmN_f1_cutoff_subfive <- calculate_F1(compareCyclaseReads(cyclaseTcmN_recoded %>% anti_join(., remove_TcmN_reads_subfive) %>% filter(HMMScore >=20),t2pksBlastDF)) #0.4661547
```
