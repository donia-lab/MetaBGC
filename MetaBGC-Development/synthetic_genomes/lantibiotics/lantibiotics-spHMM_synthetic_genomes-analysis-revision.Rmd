---
title: "terpene-synthetic_genome-analysis"
...
knitr::opts_knit$set(root.dir = '.synthetic_genomes/lantibiotics/HMM-analysis/')
```
...
##### Load the BLAST data for Terpene genes against our synthetic dataset. 
```{r, message=FALSE}
# BLAST unfiltered reads at 95% pident no readCoverage filter
all_lancBlastDF <- read_delim("../Lantibiotics-TP-reads/lantibioticBlastDF_all-results-updated.txt", col_names = T, delim = "\t")%>% anti_join(.,remove_reads, by= c("sseqid"= "readID", "Sample"))

```

##### Positional information about terpenes domains and their locations in respect to the terpene 30_10 spHMM models 
```{r, message=FALSE}
#load siderophore interval positions data 
lanc_positions <- read_tsv("../Lantibiotics-TP-reads/LanC_gene-interval_position-data-updated.txt",col_names = T)  %>% filter(start != 0)
```
...
lanc_blast_intervals<- read_tsv("../Lantibiotics-TP-reads/lanc_blast_intervals-updated.txt", col_names = T)%>% anti_join(.,remove_reads, by= c("sseqid"= "readID", "Sample"))
```
...
lanc_positions$cyclase_type <-"lantibiotic"
...
#plusfive_remaining_hmm_lanc %>% separate(readID, c("scaffold_id", "rest_id"), sep = "-", remove = F) %>% distinct(scaffold_id)
...
#parseReads(remaining_hmm_lanc_pos_median_check, "lantibiotics", "/Users/francinecamacho/Google Drive/T2PKS-paper/computational_analysis/synthetic_genomes/revision-synthetic-genomes/other-BGCs-poc/lantibiotics/HMM-analysis/hmm-unique-analysis/median_score/")

#parseReads(remaining_hmm_lanc_pos_subfive_check, "lantibiotics", "/Users/francinecamacho/Google Drive/T2PKS-paper/computational_analysis/synthetic_genomes/revision-synthetic-genomes/other-BGCs-poc/lantibiotics/HMM-analysis/hmm-unique-analysis/minus_five_score/")

#parseReads(remaining_hmm_lanc_pos_plusfive_check, "lantibiotics", "/Users/francinecamacho/Google Drive/T2PKS-paper/computational_analysis/synthetic_genomes/revision-synthetic-genomes/other-BGCs-poc/lantibiotics/HMM-analysis/hmm-unique-analysis/plus_five_score/")

```

```{r}

blast_hmm_unique <- read_tsv("/Users/francinecamacho/Google Drive/T2PKS-paper/computational_analysis/synthetic_genomes/revision-synthetic-genomes/other-BGCs-poc/lantibiotics/HMM-analysis/hmm-unique-analysis/lanc_hmm-unique_blast-results.txt", col_names = T) %>% separate(qseqid, c("scaffold_id", "rest_id"), sep = "-", remove = F) %>% filter(str_detect(sseqid, fixed(scaffold_id))) %>% select(-c(scaffold_id, rest_id))  %>% arrange(evalue) %>% group_by(qseqid, Sample, cutoff) %>% top_n(1) %>% ungroup()
...
```{r}
lanc_f1_df %>% group_by(interval) %>% top_n(1, F1) %>% ungroup() %>%  filter(F1 >=0.5) %>% arrange(interval) %>% as.data.frame()
```


```{r}
supp_fig_lanc_f1 <-ggplot(data = lanc_f1_df, mapping = aes(x = interval, y = F1, group= cutoff, colour=cutoff )) + geom_point()+
    geom_line() + ylim(0,1)+ geom_hline(yintercept=c(.50,0.1051712), linetype="dashed", 
                color = c("red","green"), size=1) + theme_pubclean() +  scale_color_npg(name="HMM Score Cutoff")

ggsave(supp_fig_lanc_f1, file="lantibiotics_synthetic_F1-final.eps", device="eps", width = 20, height = 7)
```
...
write_tsv( lanc_filtered_final %>% select(-c(window, interval))%>% group_by(readID,Sample) %>% arrange(desc(HMMScore)) %>% top_n(1, HMMScore) %>% filter(row_number() == 1) %>% ungroup(), "lantibiotics-final_data-raw.txt", col_names = T)

#rename terpene hmm_unique within the folder to detected_reads

#parseReads(subfive_false_postives, "lantibiotics-fps", "/Users/francinecamacho/Google Drive/T2PKS-paper/computational_analysis/synthetic_genomes/revision-synthetic-genomes/other-BGCs-poc/lantibiotics/HMM-analysis/false-positives-subfive/")

parseReads(lanc_filtered_final, "lantibiotics", "/Users/francinecamacho/Google Drive/T2PKS-paper/computational_analysis/synthetic_genomes/revision-synthetic-genomes/other-BGCs-poc/lantibiotics/read-quantification/sample-readIDs-files/")
```
...
results_common$cyclase_type <- "lantibiotic"
...
results_hmm_unique$cyclase_type <- "lantibiotic"
...
write_tsv(combined_lanc_results,"lanc-final_models-data.txt",col_names = T)
```
